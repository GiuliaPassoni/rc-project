...
import {summaryToTiles} from "../../utils/api.ts";
...
export function FleetPage() {
    ...
    const cellIds = cellsStatus?.map((c) => c.cellId) ?? [];
            ...
            <section className="lg:col-span-3 bg-slate-950 border border-slate-800 rounded-xl p-6 min-h-[140px] flex flex-col justify-between">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10">
                    {cellsStatus && cellsStatus.length > 0 && (
                        cellsStatus.filter((c) => c.cellId.includes(searchTerm)).map((cellStatus, i) =>
                            <CellStatusCard key={i} {...cellStatus}/>
                        )
                    )}
                </div>
            </section>
